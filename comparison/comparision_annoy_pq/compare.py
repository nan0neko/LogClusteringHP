def func1():
    ...
    timee.append(execution_time)

    pmemo.append(int(psutil.Process(os.getpid()).memory_info().rss / 1024 ** 2))
    vmemo.append(int(psutil.Process(os.getpid()).memory_info().vms / 1024 ** 2)) 
def fun2():
    import pandas as pd
    import numpy as np
    from annoy import AnnoyIndex
...
vmemo=[]
pmemo=[]
lis=["Annoy count vectorizer","Embedded model annoy","pq tf-idf"]      
for i in range(3):
    if(i==0):
        func1()
    elif(i==1):
...
import pandas as pd
import matplotlib.pyplot as plt
liss={"Method":lis,"Virtual memory":vmemo,"physical memory":pmemo}
datfram=pd.DataFrame(liss)
print(datfram)
fig, ax1 = plt.subplots(figsize=(20, 20))

tidy = datfram.melt(id_vars='Method').rename(columns=str.title)
sns.barplot(x='Method', y='Value', hue='Variable', data=tidy, ax=ax1).set_title("Memory Usage")
